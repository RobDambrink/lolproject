...
public class GetMasteryBySummonerId extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			JSONObject json = new SummonerLogica(new Hibernate(), new CouchDB()).getMasteriesByID(id);
			if(json != null)
				JSONUtility.sendJSON(response, json);
			else 
				JSONUtility.sendError(response, "Summoner or masterypages not found.");
